<?php
/**
 * The DwollaSeed class speaks to the Dwolla API.
 *
 ...
 *
 **/
class DwollaSeed extends SeedBase {
	protected $api_username, $api_password, $api_signature, $api_endpoint, $api_version, $dwolla_base_url, $error_message, $token;

	public function __construct($user_id, $connection_id, $token=false) {
		$this->settings_type = 'com.dwolla';
		...
		if ($this->getCASHConnection()) {
			...
			$this->api_endpoint    = "https://api-3t.dwolla.com/nvp";
			$this->dwolla_base_url = "https://www.dwolla.com/webscr&cmd = ";
			if ($this->settings->getSetting('sandboxed')) {
				$this->api_endpoint    = "https://api-3t.sandbox.dwolla.com/nvp";
				$this->dwolla_base_url = "https://www.sandbox.dwolla.com/webscr&cmd = ";
			}
		} else {
	...
	protected function postToDwolla($method_name, $nvp_parameters) {
		...
		if ($http_response) {
			...
			$this->setErrorMessage('could not reach Dwolla servers');
			return false;
		}
	}
	...
	public function setExpressCheckout(
		...
		$parsed_response = $this->postToDwolla('SetExpressCheckout', $nvp_parameters);
		if (!$parsed_response) {
			$this->setErrorMessage('SetExpressCheckout failed: ' . $this->getErrorMessage());
			return false;
		} else {
			// Redirect to dwolla.com.
			$token = urldecode($parsed_response["TOKEN"]);
			$dwolla_url = $this->dwolla_base_url . "_express-checkout&token=$token";
			return $dwolla_url;
		}
	}
	...
	public function getExpressCheckout() {
		if ($this->token) {
			...
			$parsed_response = $this->postToDwolla('GetExpressCheckoutDetails', $nvp_parameters);
			if (!$parsed_response) {
	...
	public function doExpressCheckout($payment_type='Sale') {
		if ($this->token) {
			...
			$parsed_response = $this->postToDwolla('DoExpressCheckoutPayment', $nvp_parameters);
	...
	public function doRefund($transaction_id, $refund_amount, $fullrefund=true,$memo=false,$currency_id='USD') {
		...
		$parsed_response = $this->postToDwolla('RefundTransaction', $nvp_parameters);
